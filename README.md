...
- **多模态视觉识别 (Multi-modal Vision)**
  - 接入智谱 GLM-4.6V-Flash 视觉大模型，**支持用户一次性上传多张食物图片**，自动精准识别所有图片中的食材或运动数据内容，并经前端 Canvas 智能压缩后无缝接入 Agent 推理流。
...
- **防刷新中断保护 (Page Unload Guard)**
  - 当 AI Agent 正在流式生成回答时，用户若尝试刷新或关闭页面，浏览器将弹出确认拦截提示，防止 SSE 流式连接意外中断导致的内容丢失与状态不一致。
- **企业级全栈工程化 (Enterprise Engineering)**
  - **后端**：FastAPI + SSE (Server-Sent Events) 实现极速流式输出。
  - **前端**：Tailwind CSS 纯原生无框架构建，支持 Markdown 实时渲染、**多图选择预览与删除**、动态图片 Lightbox 缩放、**流式输出防刷新拦截**及全套响应式交互。
...
[ 用户端 Browser ]
   ...
   │  │ (HTTP / SSE 流式响应)
   ▼  │
[ FastAPI 后端服务 ] ──(多图 Base64 列表)──> [ 视觉大模型 GLM-4V ] (批量图片识别拦截)
   │  ▲
   │  │ (状态更新与编排)
   ▼  │
