...
class ChatRequest(BaseModel):
    message: str
    image_base64: list[str] = []  # 改为列表，支持多图
...
async def analyze_image_with_llm(base64_list: list[str]) -> str:
    """使用智谱 GLM-4V-Flash 识别多张图片"""
    ...
    try:
        ...
        prompt = (
            ...
            "2. 如果图片是运动记录（如跑步配速表、心率图），请直接提取其中的核心数值。\n"
            "3. 如果是其他与健康或身体相关的内容，请列出关键指标。\n"
            "【严格要求】：直接输出提取到的核心数据或结论，绝对不要使用诸如'这是一张...的图片'等毫无意义的开场白。"
        )
        ...
        print(f"视觉识别报错: {e}")
        return f"【图片识别系统：视觉识别暂时不可用（{type(e).__name__}）】"
